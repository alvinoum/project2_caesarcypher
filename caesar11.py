...
print ('Do you wish to Encrypt or Decrypt?') # Ask if the user would like to Encrypt or Decrypt
...
def getTraslatedMessage(mode, message, key): # Encrypt or Decrypt the message with the given Key
    ...
    for symbol in message: # loops over each letter (symbol).
        if symbol.isaplha(): # only encrypts letters and ignore the others.
